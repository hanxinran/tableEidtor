...
export default class Table extends Component {
  ...
  createTable(size){
    ...
    let tableData=[];
    for(let r=0;r++;r<rowNum){
      for(let c=0;c++;c<colNum){
        tableData.push(this.createCell(rowIndex,colIndex))
      }
    }
  ...
  createCell(rowIndex,colIndex){
    ...
    return {rowIndex,colIndex,rowSpan,colSpan,}
  }
  ...
  renderRow=(cells)=>{
   return cells.map((e,i)=> <td >{e.value || ''}</td>)
  }
